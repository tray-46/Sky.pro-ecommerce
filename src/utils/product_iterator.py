...
class ProductIterator:
    ...
    def __init__(self, product_category: Category) -> None:
        ...
        self.__index = 0
        self.__logger.info("Instance of ProductIterator created")

    def __iter__(self) -> ProductIterator:
        """Return an iterator over the products list"""
        self.__index = 0
        self.__logger.debug("Returning ProductIterator, index set to 0")
        return self

    def __next__(self) -> Product:
        """Return the next product from the list or raise StopIteration"""
        self.__logger.debug("ProductIterator called")
        if self.__index < len(self.__products_list):
            ...
            self.__logger.info("ProductIterator reached the end of the list")
            raise StopIteration

    @property
    def index(self) -> int:
        """Return the current index of the product"""
        self.__logger.debug("Assessing ProductIterator index")
        return self.__index
